    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            if (Check(q))
            {
                ...
                {
                    string vendor = GetCreditCardVendor(q);
                    if (vendor != "Unknown")
                    {
                    ...
                    if (vendor != "Unknown")
                    {
                        if (GetCreditCardVendor(GenerateNextCreditCardNumber(q)) == vendor)
                        {
                            Console.WriteLine("Next valid number:");
                            Console.WriteLine(GenerateNextCreditCardNumber(q));
                        }
                        else
                            Console.WriteLine("No more card numbers available for this vendor");
                    }
                }
                else
                    Console.WriteLine("Your card number is invalid");
        ...
        {
            string check = @"^[\d]{4} ?[\d]{4} ?([\d]{4}| ?[\d]{4} ?[\d]{1,3}| ?[\d]{4} ?[\d]{4}| ?[\d]{4} ?[\d]{4} ?[\d]{1,3})$";
